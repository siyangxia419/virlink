...
test_that("input is a data frame with the first two columns being 'id' and 'pep_aa'", {
  expect_error(peptide_comparisons_vctz(peptides = peptide_df[, -1]))
  expect_error(peptide_comparisons_vctz(peptides = peptide_df[, -2]))
  expect_error(peptide_comparisons_vctz(peptides = "NNNNNNN"))
})
#> Test passed
test_that("output has the right dimensions and structure", {
  test_result1 <- peptide_comparisons_vctz(peptides = peptide_df,
  ...
  test_result2 <- peptide_comparisons_vctz(peptides = peptide_df,
